  ...
  switch (RCC->CFGR & RCC_CFGR_SWS)
  {
    ...
    case RCC_CFGR_SWS_PLL:  /* PLL used as system clock  source */
    {
      ...
      if(__HAL_RCC_GET_PLL_OSCSOURCE() != RCC_PLLSOURCE_HSI)
      {
        ...
        pllvco = (uint32_t) ((((uint32_t) HSE_VALUE * ((uint32_t) ((RCC->PLLCFGR & RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos)))) / (uint32_t)pllm);
      }
      else
      {
        /* HSI used as PLL clock source */
        pllvco = (uint32_t) ((((uint32_t) HSI_VALUE * ((uint32_t) ((RCC->PLLCFGR & RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos)))) / (uint32_t)pllm);
      }
